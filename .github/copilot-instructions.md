...
Raspberry Pi Zero 2 W は表示専用のキオスク端末（フロントエンド）のみ。表示端末ではGoogleログインは行わない。

ウィジェット:
1) 天気（既定: 姫路市。都市は設定可能）
2) Google カレンダー（共有カレンダー1つ）
3) Google タスク（共有リスト1つ）
...
- ストレージ: `./data/` 配下のJSONファイル
  - OAuthトークン（使用する場合）は `./data/tokens.json`（またはプロバイダ別）
...
- 今日、明日、今後最大7日分を表示
- 終日イベントは日ごとに上部固定
- Googleカレンダー/イベントの色を使用（プロバイダ固有の色マッピング）
...
- location: { cityName: "姫路市", country: "JP" }  // 後で変更可能
- google: placeholder（clientId, clientSecret など）(後で設定)
- weather: placeholder（プロバイダ設定）(後で設定)
...
## 外部プロバイダ（後で埋めるプレースホルダー）
### Google API（カレンダー/タスク）
- バックエンドのみOAuth（認可コードフロー）。リフレッシュトークンはJSONに保存。
- スコープ: TODO（最小権限の読み取りスコープ）
- カレンダー:
  - 共有カレンダー1つ
  - 次の7日分（今日..今日+6）
  - 色: TODO（プロバイダ固有のマッピング）
- タスク:
  - 共有タスクリスト1つ
  - 指定通りのサーバー側ソート（期限 -> 優先度 -> createdAt）
...
- /internal/services
  - /google（calendar, tasks）  // プレースホルダー。後で実装
  - /weather                 // プレースホルダー。後で実装
  - /geocode（nominatimクライアント + キャッシュ）
